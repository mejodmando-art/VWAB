...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = "Bot VWAP Trend Momentum\n\nSettings:\nSymbol: " + settings['symbol'] + "\nTimeframe: " + settings['timeframe'] + "\nAmount: $" + str(settings['trade_amount']) + "\nSL: " + str(settings['sl_mult']) + "x ATR\nTP: " + str(settings['tp_mult']) + "x ATR\nStatus: " + ("Running" if settings['active'] else "Stopped")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        settings = db.get_settings(chat_id)
        text = "Settings:\nAmount: $" + str(settings['trade_amount']) + "\nTF: " + settings['timeframe'] + "\nSymbol: " + settings['symbol'] + "\nSL: " + str(settings['sl_mult']) + "x\nTP: " + str(settings['tp_mult']) + "x\nEMA: " + str(settings['ema_len']) + "\nPullback: " + str(settings['pullback_dist']) + "%\nLong Only: " + ("Yes" if settings['long_only'] else "No")
        ...
        if not trades:
            ...
            text = "Open Trades:\n\n"
            for t in trades:
                text += t[3] + " " + t[2] + "\nEntry: " + str(round(float(t[4]), 4)) + "\nSL: " + str(round(float(t[5]), 4)) + " | TP: " + str(round(float(t[6]), 4)) + "\nQty: " + str(round(float(t[7]), 4)) + "\n\n"
        await query.edit_message_text(text, reply_markup=main_menu())
    elif data == "history":
        ...
        if not trades:
            ...
            for t in trades:
                ...
                text += t[3] + " " + t[2] + " PNL: " + str(round(pnl, 2)) + "\n"
        await query.edit_message_text(text, reply_markup=main_menu())
    elif data == "start_bot":
...
async def show_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        balance = "Error"
        price = "Error"
    text = "Bot Status\n\nStatus: " + ("Running" if settings['active'] else "Stopped") + "\nBalance: " + (str(balance) if isinstance(balance, str) else str(round(balance, 2)) + " USDT") + "\nPrice: " + (str(price) if isinstance(price, str) else str(round(price, 4))) + "\nOpen Trades: " + str(len(open_trades)) + "\nDaily Trades: " + str(daily_count) + "/" + str(settings['max_trades_day']) + "\n\nSettings:\nSymbol: " + settings['symbol'] + " | TF: " + settings['timeframe'] + "\nAmount: $" + str(settings['trade_amount']) + "\nSL: " + str(settings['sl_mult']) + "x | TP: " + str(settings['tp_mult']) + "x"
...
async def check_signal(chat_id: int) -> str:
    ...
    try:
        ...
        if result['signal']:
            ...
            return side + " trade opened!\nSymbol: " + settings['symbol'] + "\nEntry: " + str(round(entry, 4)) + "\nSL: " + str(round(sl, 4)) + "\nTP: " + str(round(tp, 4)) + "\nATR: " + str(round(result['atr'], 4)) + "\nQty: " + str(round(amount, 6)) + "\nR:R = " + str(settings['tp_mult']) + ":" + str(settings['sl_mult'])
        else:
