...
from strategy import EMA9Cross21Strategy
...
def main_menu():
    keyboard = [
        ...
        [InlineKeyboardButton("🔍 فحص 500 عملة", callback_data="scan_market"),
         InlineKeyboardButton("📈 الفرص", callback_data="opportunities")],
        [InlineKeyboardButton("▶️ تشغيل تلقائي", callback_data="start_bot"),
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome = "🤖 بوت EMA 9/21 Crossover" + chr(10) + chr(10) + "📊 الاستراتيجية:" + chr(10) + "• EMA 9 يخترق EMA 21 (1H)" + chr(10) + "• تأكيد من 4H" + chr(10) + chr(10) + "⚙️ الإعدادات:" + chr(10) + "• المبلغ: $" + str(settings["trade_amount"]) + chr(10) + "• SL: " + str(settings["sl_mult"]) + "x ATR" + chr(10) + "• TP: " + str(settings["tp_mult"]) + "x ATR" + chr(10) + "• Max صفقات: " + str(settings["max_open_trades"]) + chr(10) + "• الحالة: " + ("🟢 شغال" if settings["active"] else "🔴 متوقف")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "main_menu":
        ...
        settings = db.get_settings(chat_id)
        text = "⚙️ الإعدادات:" + chr(10) + chr(10) + "💵 المبلغ: $" + str(settings["trade_amount"]) + chr(10) + "🛑 SL: " + str(settings["sl_mult"]) + "x ATR" + chr(10) + "🎯 TP: " + str(settings["tp_mult"]) + "x ATR" + chr(10) + "📈 Max صفقات: " + str(settings["max_open_trades"]) + chr(10) + "🔒 Long Only: " + ("نعم" if settings["long_only"] else "لا") + chr(10) + chr(10) + "اضغط لتعديل:"
        ...
        if not trades:
            ...
            for t in trades:
                text += ("🟢" if t[3] == "LONG" else "🔴") + " " + t[3] + " " + t[2] + chr(10) + "الدخول: " + str(round(float(t[4]), 4)) + chr(10) + "SL: " + str(round(float(t[5]), 4)) + " | TP: " + str(round(float(t[6]), 4)) + chr(10) + "الكمية: " + str(round(float(t[7]), 4)) + chr(10) + chr(10)
        await query.edit_message_text(text, reply_markup=main_menu())
    elif data == "history":
        ...
        await query.edit_message_text("✅ البوت شغال! سيفحص 500 عملة كل ساعة ويدخل صفقات أوتوماتيك.", reply_markup=main_menu())
        ...
        if scanning:
            ...
            scanning = True
            await query.edit_message_text("🔍 جاري فحص 500 عملة...", reply_markup=main_menu())
...
async def show_report(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "📊 التقرير" + chr(10) + chr(10) + "💰 ربح/خسارة اليوم: " + str(round(today_pnl, 2)) + " USDT" + chr(10) + "📈 إجمالي الربح: " + str(round(total_pnl, 2)) + " USDT" + chr(10) + "📊 إجمالي الصفقات: " + str(total_trades) + chr(10) + "🟢 الصفقات الرابحة: " + str(winning_trades) + chr(10) + "🔴 نسبة النجاح: " + str(round(win_rate, 1)) + "%"
    if hasattr(update, "callback_query") and update.callback_query:
        await update.callback_query.edit_message_text(text, reply_markup=main_menu())
    else:
        await update.message.reply_text(text, reply_markup=main_menu())
...
async def show_opportunities(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = "📈 الفرص الأخيرة:" + chr(10) + chr(10) + "اضغط 🔍 فحص 500 عملة للحصول على فرص جديدة"
    if hasattr(update, "callback_query") and update.callback_query:
        await update.callback_query.edit_message_text(text, reply_markup=main_menu())
    else:
        await update.message.reply_text(text, reply_markup=main_menu())
...
async def scan_market_task(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        settings = db.get_settings(chat_id)
        await context.bot.send_message(chat_id=chat_id, text="🔍 جاري تحميل قائمة 500 عملة...")

        coins = gateio.get_top_coins(500)
        if not coins:
            await context.bot.send_message(chat_id=chat_id, text="❌ لم يتم العثور على عملات")
            scanning = False
            return

        db.save_top_coins(coins)
        await context.bot.send_message(chat_id=chat_id, text="✅ تم العثور على " + str(len(coins)) + " عملة. جاري الفحص على فريم 1H + تأكيد 4H...")

        strategy = EMA9Cross21Strategy(
        ...
        for coin in coins:
            try:
                candles_1h = gateio.get_klines(coin["symbol"], "1h", 200)
                candles_4h = gateio.get_klines(coin["symbol"], "4h", 100)

                if len(candles_1h) < 100 or len(candles_4h) < 50:
                    continue

                result = strategy.analyze_1h(candles_1h, candles_4h)
        ...
        if not opportunities:
            await context.bot.send_message(chat_id=chat_id, text="⏳ لا توجد فرص EMA Cross حالياً من " + str(scanned) + " عملة.")
        else:
            text = "🎯 أفضل " + str(min(10, len(opportunities))) + " فرص EMA 9/21 Cross:" + chr(10) + chr(10)
            for i, opp in enumerate(opportunities[:10]):
                ...
                text += "   EMA9: " + str(round(opp["ema9"], 4)) + " | EMA21: " + str(round(opp["ema21"], 4)) + chr(10)
                text += "   SL: " + str(round(opp["sl"], 4)) + " | TP: " + str(round(opp["tp"], 4)) + chr(10) + chr(10)

            await context.bot.send_message(chat_id=chat_id, text=text)

            if settings["active"]:
                await auto_trade_opportunities(chat_id, context, opportunities[:5], settings)
...
async def auto_trade_opportunities(chat_id: int, context: ContextTypes.DEFAULT_TYPE, opportunities: list, settings: dict):
    ...
    for opp in opportunities:
        ...
        try:
            ...
            trade_id = db.add_trade(chat_id, opp["symbol"], opp["signal"], opp["price"], opp["sl"], opp["tp"], amount, opp["atr"], opp["ema9"], opp["ema21"])
            db.increment_daily_count(chat_id)
            db.add_signal(chat_id, opp["symbol"], opp["signal"], opp["price"], opp["ema9"], opp["ema21"], opp["atr"], opp["strength"], "1H+4H")
