...
class Database:
        ...
        with self.conn.cursor() as cur:
            ...
            cur.execute("""
                CREATE TABLE IF NOT EXISTS settings (
                    ...
                    symbol VARCHAR(20) DEFAULT 'BTC_USDT',
                    timeframe VARCHAR(10) DEFAULT '15m',
                    trade_amount DECIMAL(20,8) DEFAULT 50.0,
                    max_trades_day INT DEFAULT 6,
                    sl_mult DECIMAL(5,2) DEFAULT 2.0,
                    tp_mult DECIMAL(5,2) DEFAULT 2.0,
                    long_only BOOLEAN DEFAULT true,
                    ema_len INT DEFAULT 34,
                    pullback_dist DECIMAL(5,2) DEFAULT 0.15,
    ...
    def update_settings(self, chat_id, **kwargs):
        with self.conn.cursor() as cur:
            for key, val in kwargs.items():
                cur.execute(f"UPDATE settings SET {key}=%s WHERE chat_id=%s", (val, chat_id))
            self.conn.commit()
    ...
    def increment_daily_count(self, chat_id):
        with self.conn.cursor() as cur:
            cur.execute("""
                INSERT INTO daily_trades (chat_id, count) VALUES (%s, 1)
                ON CONFLICT (chat_id, trade_date) DO UPDATE SET count = daily_trades.count + 1
            """, (chat_id,))
            self.conn.commit()
    ...
    def add_trade(self, chat_id, symbol, side, entry, sl, tp, amount):
        with self.conn.cursor() as cur:
            cur.execute("""
                INSERT INTO trades (chat_id, symbol, side, entry_price, stop_loss, take_profit, amount)
                VALUES (%s,%s,%s,%s,%s,%s,%s) RETURNING id
            """, (chat_id, symbol, side, entry, sl, tp, amount))
            self.conn.commit()
    ...
    def close_trade(self, trade_id, close_price, pnl):
        with self.conn.cursor() as cur:
            cur.execute("""
                UPDATE trades SET status='CLOSED', close_price=%s, pnl=%s, closed_at=CURRENT_TIMESTAMP
                WHERE id=%s
    ...
    def get_trade_history(self, chat_id, limit=10):
        with self.conn.cursor() as cur:
            cur.execute("""
                SELECT * FROM trades WHERE chat_id=%s ORDER BY opened_at DESC LIMIT %s
            """, (chat_id, limit))
            return cur.fetchall()
    ...
    def add_signal(self, chat_id, symbol, signal_type, price, vwap, ema_slope, atr):
        with self.conn.cursor() as cur:
            cur.execute("""
                INSERT INTO signals (chat_id, symbol, signal_type, price, vwap, ema_slope, atr)
                VALUES (%s,%s,%s,%s,%s,%s,%s)
            """, (chat_id, symbol, signal_type, price, vwap, ema_slope, atr))
            self.conn.commit()
